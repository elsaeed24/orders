            ...
            <div class="col-md-3">
                <div class="card">
                    ...
                    <div class="card-body text-right ">
                        <ul class="list-group ">
                            <a href="{{ route('meal_index') }}" class="list-group-item list-group-item-action ">عرض
                                الوجبات</a>
                            <a href="{{ route('meal_show') }}" class="list-group-item list-group-item-action">إضافة
                                وجبة</a>
                            ...
                            <tbody>
                              ...
                              @if (count($meals) > 0)
                                @foreach ($meals as $row)
                                    ...
                                    <tr>
                                        <th scope="row">{{ ++$i }}</th>
                                        <td><img src="{{ asset($row->photo) }}" width="80"></td>
                                        <td>{{$row->name  }}</td>
                                        <td>{{$row->description }}</td>
                                        <td>{{$row->category }}</td>
                                        <td>{{$row->price . '$' }}</td>

                                        <td><a href="{{ route('meal_edit', $row->id) }}"><button class="btn btn-primary">تعديل</button></a></td>
